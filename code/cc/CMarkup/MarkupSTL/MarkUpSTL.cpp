...
//lint -esym(1529,CMarkupSTL::operator=)
void CMarkupSTL::operator=( const CMarkupSTL& markup )
{
    ...
    //zc Ìí¼Ó 20110823
    std::map<std::string,SavedPos> ::const_iterator iter;
    for(iter = markup.m_mapSavedPos.begin();iter != markup.m_mapSavedPos.end();iter++)
    {
        m_mapSavedPos.insert(mapSavedPosT::value_type(iter->first, iter->second));
    }
